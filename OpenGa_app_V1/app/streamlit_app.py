...
st.set_page_config(page_title="OpenGa", page_icon="⚗️", layout="wide",
...
TABS = ["Dashboard", "Inputs", "Mass & energy", "Equipment & CAPEX", "Operating cost",
                        ...
                        with cols[1]:
                            if baseline:
                                ...
                                v = st.selectbox(label, opts, index=opts.index(int(cur))
                                                 ...
                                                 key=f"in_{key}", label_visibility="collapsed")
                                if float(v) != float(m["default"]):
                                    st.session_state.overrides[key] = float(v)
                                else:
                                    st.session_state.overrides.pop(key, None)
                            else:
                                shown = cur * 100 if pct else cur
                                v = st.number_input(
                                    label, value=float(shown), key=f"in_{key}",
                                    label_visibility="collapsed", format="%.6g")
                                real = v / 100 if pct else v
                                if abs(real - m["default"]) > 1e-15:
                                    st.session_state.overrides[key] = real
                                else:
                                    st.session_state.overrides.pop(key, None)
                        with cols[2]:
...
with tabs[10]:
    ...
    for gname, gtitle, blurb in [
        ...
        ("regression", "Workbook regression",
         "Does the port still reproduce OpenGa_v3.3_Carbon.xlsx? A failure means the "
